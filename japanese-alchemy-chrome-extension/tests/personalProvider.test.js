import {
  ...
  initializePersonalProviderState,
  normalizeApiBaseUrl,
...
describe('personal provider state', () => {
  ...
  test('the restoration epoch discards legacy provider data and analysis cache once', async () => {
    const catalogKey = `${PERSONAL_PROVIDER_CATALOG_KEY_PREFIX}9`;
    const { store } = setupChrome({
      ...
      [catalogKey]: { modelIds: ['legacy-model'] },
      [PERSONAL_PROVIDER_PENDING_PERMISSION_CLEANUP_KEY]: [],
      lastAnalysisKey: 'legacy-cache-key',
      lastResponse: '# legacy response',
      lastAnalysisResult: { cacheKey: 'legacy-cache-key' },
      selectedText: '日本語',
      contextBefore: '前',
      contextAfter: '後',
    }, ['https://api.example.test/*']);

    await expect(initializePersonalProviderState()).resolves.toEqual(
      expect.objectContaining({ epoch: 2 })
    );
    expect(store).toEqual(expect.objectContaining({
      [PERSONAL_PROVIDER_EPOCH_KEY]: 2,
    ...
    store[ANALYSIS_PROVIDER_MODE_KEY] = PERSONAL_PROVIDER_MODE;
    await initializePersonalProviderState();
