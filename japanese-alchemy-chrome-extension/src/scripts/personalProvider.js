...
export const PERSONAL_PROVIDER_EPOCH_KEY = 'personalProviderEpoch';
...
const RESTORATION_PROVIDER_EPOCH = 2;
...
export async function initializePersonalProviderState() {
  return withProviderMutationLock(async () => {
    await restrictLocalStorageToTrustedContexts();
    const local = requireLocalStorage();
    const stored = await local.get(null);
    if (stored[PERSONAL_PROVIDER_EPOCH_KEY] === RESTORATION_PROVIDER_EPOCH) {
      return { epoch: RESTORATION_PROVIDER_EPOCH };
    }

    const catalogKeys = Object.keys(stored).filter((key) => (
      key.startsWith(PERSONAL_PROVIDER_CATALOG_KEY_PREFIX)
    ));
    const legacyKeys = [
    ...
    let retiredOriginPermission = null;
    try {
      retiredOriginPermission = getOriginPermission(stored[PERSONAL_PROVIDER_PROFILE_KEY]?.apiUrl);
    } catch {
      retiredOriginPermission = null;
    }
    const pendingPermissionCleanup = nextPendingPermissionOrigins(
      stored[PERSONAL_PROVIDER_PENDING_PERMISSION_CLEANUP_KEY],
      retiredOriginPermission,
      null
    );

    await local.remove(legacyKeys);
    await local.set({
      [PERSONAL_PROVIDER_EPOCH_KEY]: RESTORATION_PROVIDER_EPOCH,
    ...
    return {
      epoch: RESTORATION_PROVIDER_EPOCH,
      ...maintenance,
    };
  });
}
...
export async function getPersonalProviderState({ performMaintenance = true } = {}) {
  ...
  const stored = await getStoredProviderValues();
  const epoch = stored?.[PERSONAL_PROVIDER_EPOCH_KEY] === RESTORATION_PROVIDER_EPOCH
    ? RESTORATION_PROVIDER_EPOCH
    : null;
  ...
  const readiness = await readProfileReadiness(epoch === RESTORATION_PROVIDER_EPOCH
    ? stored?.[PERSONAL_PROVIDER_PROFILE_KEY]
    : null);
